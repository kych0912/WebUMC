- React의 동작 원리
    ...
    - 정리
        ...
        SPA는 단일 페이지 애플리케이션(Single Page Application)으로 서버에서 필요한 데이터만 비동기로 받아와서 동적으로 현재 화면에 다시 렌더링 하는 방식
        
    1. User Interface Library 
    - 정리
        
        프런트엔드에서 작동하는 다양한 컴포넌트를 묶어 직접 구현하지 않아도 바로 사용할 수 있도록 제공하는 라이브러리
        
        ex - material UI, tailwind, bootstrap
    ...
    - 정리
        ...
        클래스 컴포넌트에 비해 리렌더링 될 때 immutable 하다. (class component는 this가 변경 가능)
        
        props의 destructing으로 가독성 또한 높고, render 함수 선언, 상속 생성자 등 이러한 코드들을 작성하지 않아도 된다.
        
    1. Virtual DOM (가상 DOM)
    - 정리
        
        가상 DOM은 실제 DOM을 작업하지 않고 가상 DOM을 구성하여 diff 부분반 rerender한다.
        
    1. 동시성 렌더링
    - 정리
        - **긴급한 업데이트(Urgent updates)와 전환 업데이트(Transition updates)**
            - 만약 변경사항이 있으면 긴급한 업데이트가 우선순위를 가지고 변경된다.
            - 전환 업데이트는 후순위로 작업한다.
            - useState의 set함수와 같은 경우 Urgent updates
        - **`startTransition`**
            - 안에 들어있는 작업은 후순위를 가지게 되어 Urgent update가 있는 경우 멈춘다.
            - 함수 내부에는 한 개 이상의 set함수가 존재해야 한다.
        - **`useTransition`**
            - `const [isPending, startTransition] = useTransition()`
            - 위와 같이 지연중인 전환이 있느지를 확인할 수 있다.
        - **`useDeferredValue`**
            - `const deferredValue = useDeferredValue(value)`
            - set에 접근하기 어려울 때 사용한다.
            - value와 deferredValue를 비교하여 지연중임을 확인할 수 있다.
...
- 얕은 복사 🍠
    ...
    얕은 복사는 변수가 저장된 메모리 address를 저장하는 것이다. 그래서 값이 변경되면 복사된 값, 복사한 값 둘 다 변경된다.
    
- 깊은 복사 🍠
    
    새로운 메모리 주소를 할당하여 변수 내에있는 값을 복사하는 것이다.